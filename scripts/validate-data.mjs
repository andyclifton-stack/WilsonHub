...
const ids=new Set()
for(const r of records){if(ids.has(r.id))errors.push(`Duplicate stable ID: ${r.id}`);ids.add(r.id);for(const field of ['created','firstSeen','lastChecked','lastUpdated']){if(Number.isNaN(Date.parse(`${r[field]}T00:00:00Z`)))errors.push(`${r.id}.${field} is not a valid date`);if(r[field] > '2026-08-06')errors.push(`${r.id}.${field} is in the future relative to this content release`)}}
